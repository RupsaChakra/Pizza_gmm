...
tb_path='/common/usersc/rc1195/final_pascal_voc/'
writer=SummaryWriter("runs/pascal_voc")
...
parser.add_argument('--dataset', default='VOC300', choices=['VOC300', 'VOC512','COCO'],
...
def train():
    ...
    print(args.start_iter,cfg['max_iter'])
    for iteration in range(args.start_iter, cfg['max_iter']):
        if args.visdom and iteration != 0 and (iteration % epoch_size == 0):
            # update_vis_plot(epoch, loc_loss, conf_loss, epoch_plot, None,
            #                 'append', epoch_size)
            print(iteration,epoch_size)
            #Iteration vs Loss
            loc_loss = 0
            conf_loss = 0
            epoch += 1
        ...
        if iteration % 10 == 0:
            ...
            X=iteration
            Y=loc_loss / epoch_size
            writer.add_scalar('Localization_Loss/train',Y,X)
            # reset epoch loss counters
            y=conf_loss/epoch_size
            writer.add_scalar('Conf_Loss/train',y,X)
            z=Y+y
            writer.add_scalar('Total_loss/train',z,X)

        if iteration != 0 and (iteration+1) % 10000 == 0:
            print('Saving state, iter:', iteration)
            torch.save(net.state_dict(), 'weights/ssd300' + '_id_' + str(args.id) + '_' + repr(iteration + 1) + '_voc.pth')
